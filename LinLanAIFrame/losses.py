from .structs import *
from .ImageGenerator.GANModels import Discriminator
...
def hinge(fake_sample, true_sample):
...
def discriminator_gradient_penalty_loss(discriminator: Discriminator, fake_sample,
                                        true_sample, create_graph=True,
    ...
    true_score = discriminator(true_sample)
...
class VQAutoEncoderLoss(nn.Module):
    ...
    def forward(self, fake_sample, true_sample, book_loss, log_var: torch.Tensor = None):
...
class AutoEncoderKLLoss(nn.Module):
    ...
    def forward(self, fake_sample: torch.Tensor, true_sample: torch.Tensor, latent: DiagonalGaussianDistribution,
        ...
        return (rec_loss + kl * self.kl_weight) / batch_size
