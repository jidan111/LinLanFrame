...
class Discriminator(ConfigModule):
    def __init__(self, image_shape=(3, 64, 64), hidden_channels=8, depth=None, ):
...
class Generator(ConfigModule):
    def __init__(self, in_dim, image_shape, hidden_channels=8, depth=None, attention=[], head_num=8,
                 dropout=.1, resnet_num=1, up_mode="ConvTranspose2d"):
        ...
        self.out_conv = nn.Conv2d(in_channels=hidden_channels, out_channels=image_shape[0], kernel_size=3, stride=1, padding=1)
...
class UnetDiscriminator(ConfigModule):
    def __init__(self, in_channels, depth=3, hidden_channels=4, resnet_num=2, ):
