...
class Diffusion(ConfigModule):
    ...
    def forward(self, x, condition1=None, condition2=None):
        ...
        pre_noise = self.model(xt, t_embed, condition1, condition2)
        loss = self.loss(pre_noise, noise)
        return loss

    @torch.no_grad()
    def __clean_noise_p(self, xt, t, condition1=None, condition2=None):
        index = t
    ...
    @torch.no_grad()
    def __sample_ddpm(self, batch_size, image_shape, condition1=None, condition2=None):
        x = torch.randn(size=(batch_size, *image_shape), device=self.device)
        for i in tqdm(range(self.step_nums - 1, -1, -1), desc="DDPM Sampling"):
            x = self.__clean_noise_p(x, i, condition1, condition2)
        return x

    @torch.no_grad()
    def __clean_noise_i(self, xt, t1, t2, condition1=None, condition2=None, sigma=0.):
        """t1->t2"""
        index = t2
        batch_size = xt.shape[0]
        ...
        t1_pre_noise = self.model(xt, t_embed, condition1, condition2)
    ...
    @torch.no_grad()
    def __sample_ddim(self, batch_size, image_shape, x=None, condition1=None, condition2=None, step=2, sigma=0.):
        steps_arr = list(range(self.step_nums - 1, -1, -step))
        if x is None:
            x = torch.randn(size=(batch_size, *image_shape), device=self.device)
        for i in tqdm(steps_arr, desc="DDIM Sampling"):
            next_t = max(i - step, 0)
            x = self.__clean_noise_i(x, i, next_t, condition1, condition2, sigma=sigma)
            if next_t == 0:
                break
        return x

    @torch.no_grad()
    def __clean_noise_dpm_2m(self, xt, t0_pre_noise=None, t1=0, t2=0, condition1=None, condition2=None):
        """t1->t2"""
        batch_size = xt.shape[0]
    ...
    @torch.no_grad()
    def __sample_dpm_2m(self, batch_size, image_shape, x=None, condition1=None, condition2=None, step=5):
        steps_arr = list(range(self.step_nums - 1, -1, -step))
        if x is None:
            x = torch.randn(size=(batch_size, *image_shape), device=self.device)
        t0_pre_noise = None
        for i in tqdm(steps_arr, desc="DPM++ Sampling"):
            next_t = max(i - step, 0)
            x, t0_pre_noise = self.__clean_noise_dpm_2m(xt=x, t0_pre_noise=t0_pre_noise, t1=i, t2=next_t,
