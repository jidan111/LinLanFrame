...
class Diffusion(ConfigModule):
    ...
    def forward(self, x, condition=None):
        ...
        pre_noise = self.model(xt, t_embed, condition)
        loss = self.loss(pre_noise, noise)
        return loss

    @torch.no_grad()
    def __clean_noise_p(self, xt, t, condition=None):
        index = t
    ...
    @torch.no_grad()
    def __sample_ddpm(self, batch_size=4, condition=None):
        ...
        for i in tqdm(range(self.step_nums - 1, -1, -1), desc="DDPM Sampling"):
            x = self.__clean_noise_p(xt=x, t=i, condition=condition)
        return x

    @torch.no_grad()
    def __clean_noise_i(self, xt, t1, t2, condition=None, sigma=0.):
        """t1->t2"""
        index = t2
        batch_size = xt.shape[0]
        ...
        t1_pre_noise = self.model(xt, t_embed, condition)
    ...
    @torch.no_grad()
    def __sample_ddim(self, batch_size=4, x=None, condition=None, step=2, sigma=0.):
        steps_arr = list(range(self.step_nums - 1, -1, -step))
        if x is None:
            x = torch.randn(size=(batch_size, *self.image_shape), device=self.device)
        for i in tqdm(steps_arr, desc="DDIM Sampling"):
            next_t = max(i - step, 0)
            x = self.__clean_noise_i(xt=x, t1=i, t2=next_t, condition=condition, sigma=sigma)
            if next_t == 0:
                break
        return x

    @torch.no_grad()
    def __clean_noise_dpm_2m(self, xt, t0_pre_noise=None, t1=0, t2=0, condition=None):
        """t1->t2"""
        batch_size = xt.shape[0]
    ...
    @torch.no_grad()
    def __sample_dpm_2m(self, batch_size=4, x=None, condition=None, step=5):
        steps_arr = list(range(self.step_nums - 1, -1, -step))
        if x is None:
            x = torch.randn(size=(batch_size, *self.image_shape), device=self.device)
        t0_pre_noise = None
        for i in tqdm(steps_arr, desc="DPM++ Sampling"):
            next_t = max(i - step, 0)
            x, t0_pre_noise = self.__clean_noise_dpm_2m(xt=x, t0_pre_noise=t0_pre_noise, t1=i, t2=next_t, condition=condition)
            if next_t == 0:
                break
        return x

    @torch.no_grad()
    def sample(self, batch_size=4, condition=None, mode="ddpm", x=None, step=5, sigma=0.):
        assert mode in ["ddpm", "ddim", "dpm"], "支持 ddpm/ddim/dpm 采样"
        if mode == "ddpm":
            return self.__sample_ddpm(batch_size=batch_size, condition=condition)
        elif mode == "ddim":
            return self.__sample_ddim(batch_size=batch_size, x=x, condition=condition, step=step, sigma=sigma)
        else:
            return self.__sample_dpm_2m(batch_size=batch_size, x=x, condition=condition, step=step)
