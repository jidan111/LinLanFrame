...
class AutoEncoder(ConfigModule):
    def __init__(self, in_channels, depth, hidden_channels, latent_dim, attention=[], dropout=.1, resnet_num=1,
        ...
        self.init = nn.Conv2d(in_channels=in_channels, out_channels=hidden_channels, kernel_size=1)
        ...
        self.p_dec = nn.Conv2d(in_channels=latent_dim, out_channels=self.hidden_channels * (2 ** self.depth),
                               kernel_size=1)
        self.out = nn.Conv2d(in_channels=hidden_channels, out_channels=in_channels, kernel_size=3, stride=1,
                             padding=1)
    ...
    @torch.no_grad()
    def sample(self, batch_size, latent_shape=None):
        assert latent_shape is not None, '需要设置latent_shape'
        device = self.init.weight.device
        noise = torch.randn(size=(batch_size, *latent_shape), device=device)
        out = self.latent2image(noise)
        return out


class VQBridgeProjectorAttention(nn.Module):
    def __init__(self, embed_num, embed_dim, head_nums=4, layer_num=2, dropout=.1, mul_size=16):
...
class VectorQuantizer(nn.Module):
    def __init__(self, embed_num, embed_dim, beta=0.25,
                 ...
                 vq_bridge_dropout=.1, vq_bridge_type="attn"):
...
class VQAutoEncoder(ConfigModule):
    def __init__(self, in_channels, depth, hidden_channels, latent_dim, attention=[], dropout=.1, resnet_num=1,
                 ...
                 vq_bridge_mul_size=16, vq_bridge_head_num=4, vq_bridge_layer_num=2,
                 vq_bridge_dropout=.1, vq_bridge_type="atte"):
        ...
        self.p_dec = nn.Conv2d(in_channels=latent_dim, out_channels=self.hidden_channels * (2 ** self.depth),
                               kernel_size=1)
        self.book = VectorQuantizer(embed_num=embed_num, embed_dim=latent_dim, beta=beta, use_vq_bridge=use_vq_bridge,
                                    vq_bridge_mul_size=vq_bridge_mul_size, vq_bridge_head_num=vq_bridge_head_num,
                                    vq_bridge_layer_num=vq_bridge_layer_num,
                                    vq_bridge_dropout=vq_bridge_dropout, vq_bridge_type=vq_bridge_type)
        self.out = nn.Conv2d(in_channels=hidden_channels, out_channels=in_channels, kernel_size=3, stride=1,
                             padding=1)
    ...
    def sample(self, batch_size, latent_shape=None):
        assert latent_shape is not None, '需要设置latent_shape'
        device = self.init.weight.device
        noise = torch.randint(low=0, high=self.embed_num, size=(batch_size, *latent_shape),
                              dtype=torch.long, device=device)
        out = self.book.get_codebook_entry(noise, shape=(batch_size, latent_shape[1], latent_shape[2], latent_shape[0]))
