...
class BPE(object):
    def __init__(self, vocab_size_limit=2048, load=True):
        ...
        self.pad, self.start, self.end, self.unk, self.w_end = "</PAD>", "</START>", "</END>", "</UNK>", "</w>"
        self.encoding_map = {}
        self.decoding_map = {}
    ...
    def __reorder_encoding_map(self):
        special = [self.pad, self.start, self.end, self.unk, self.w_end]
        ...
        self.encoding_map = {token: idx for idx, token in
                             enumerate([self.pad, self.start, self.w_end, self.unk] + train_tokens + [self.end])}
        self.decoding_map = {v: k for k, v in self.encoding_map.items()}

    def train(self, text, file_name="./bpe.json"):
    ...
    def __encode_word(self, word):
        ...
        ptr = len(word)
        tokens = []
        unk_id = self.encoding_map[self.unk]
        while ptr > 0:
            for i in range(ptr):
                cur = word[i:ptr]
    ...
    def __encode_sentence(self, sentence: str, dim=128):
        start = self.encoding_map[self.start]
        end = self.encoding_map[self.end]
        ...
        tokens.extend([self.encoding_map[self.pad]] * (dim - len(tokens)))
        return tokens
    ...
    def decode_tokens(self, tokens):
        ...
        for token in tokens:
            ...
            sentence.append("".join(words).replace(self.w_end, " "))
        return sentence
...
class Tokenizer(object):
    def __init__(self, dim=256, numpy=True):
        self.bpe = BPE(load=True)
        self.dim = dim
        self.numpy = numpy

    def __call__(self, text_array, dim=None):
        if type(text_array) == str:
            text_array = [text_array]
        if dim is None:
            dim = self.dim
        return self.bpe.encode_sentences(text_array, dim=dim, numpy=self.numpy)

    def decode_tokens(self, tokens):
        return self.bpe.decode_tokens(tokens)
