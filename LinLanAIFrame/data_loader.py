...
class ImageWithCaptionH5DataBase(object):
    def __init__(self, file_name, image_shape=(8, 64, 64),
                 caption_dim=128, image_dtype=np.uint8, caption_dtype=np.int16):
        self.file_name = file_name
        self.image_dtype = image_dtype
        self.caption_dtype = caption_dtype
        self.compression = "gzip"
    ...
    def recommend_chunk_size_per_sample(self):
        ...
        dtype_size = {
            ...
            np.int16: 2,
            np.uint8: 1
        }.get(self.caption_dtype, 4)
        # 2. 计算单样本总字节数 (KB)
        latent_bytes = np.prod(self.image_shape) * dtype_size
        text_bytes = self.caption_dim * dtype_size
    ...
    def create_h5(self):
        with h5py.File(self.file_name, "w") as f:
            f.create_dataset(
                ...
                maxshape=(None, *self.image_shape),  # 允许动态扩展
                chunks=(self.chunk_size, *self.image_shape),  # 关键：按批量对齐chunks
            ...
            f.create_dataset(
                "captions",
                shape=(0, self.caption_dim),
                maxshape=(None, self.caption_dim),
                chunks=(self.chunk_size, self.caption_dim),
                dtype=self.caption_dtype,
                compression=self.compression,
                compression_opts=6,
                shuffle=True  # 额外缩体积
            )
            # 初始化有效样本计数器（原子操作的关键）
            f.attrs["caption_cnt"] = 0  # 安全写入起点
            f.attrs["image_cnt"] = 0

    def append_batch(self, images, captions):
        batch_size = images.shape[0]
        if batch_size % self.chunk_size != 0:
            warnings.warn("batch_size必须能被chunk_size整除，否则存储时间与文件体积将会变大")
        assert batch_size > 0, "批次必须大于0"
        assert images.shape == (batch_size, *self.image_shape), f"Latents 形状和预设不同: {images.shape}"
        assert captions.shape == (
            batch_size, self.caption_dim), f"text_embeds 形状和预设不同: {captions.shape}"
        with h5py.File(self.file_name, "a") as f:  # 标准读写模式
            image_current_idx = f.attrs["image_cnt"]
            caption_current_idx = f.attrs["caption_cnt"]
            image_new_idx = image_current_idx + batch_size
            caption_new_idx = caption_current_idx + batch_size
            try:
                # === 原子操作：一次性扩展并写入 ===
                f["images"].resize(image_new_idx, axis=0)
                f["captions"].resize(caption_new_idx, axis=0)
                # 严格保持配对关系（HDF5保证写入原子性）
                f["images"][image_current_idx:image_new_idx] = images
                f["captions"][caption_current_idx:caption_new_idx] = captions
                # 仅当全部成功才更新计数器
                f.attrs["image_cnt"] = image_new_idx
                f.attrs["caption_cnt"] = caption_new_idx
                f.flush()  # 强制落盘
            except Exception as e:
                # 事务回滚：恢复到安全状态
                f["images"].resize(image_current_idx, axis=0)
                f["captions"].resize(caption_current_idx, axis=0)
                raise RuntimeError(f"写入失败! 指针回滚到安全状态") from e

    def append_images_batch(self, images):
        batch_size = images.shape[0]
        if batch_size % self.chunk_size != 0:
            warnings.warn("batch_size必须能被chunk_size整除，否则存储时间与文件体积将会变大")
        assert batch_size > 0, "批次必须大于0"
    ...
    def append_captions_batch(self, captions):
    ...
    def read_data(self, index, batch_size=None):
        with h5py.File(self.file_name, "r") as f:
            if batch_size is None:
                return (
                    f["images"][index],  # shape (8, 64, 64)
                    f["captions"][index]  # shape (80, 512)
                )
            else:
                return (
                    f["images"][index:index + batch_size],
                    f["captions"][index:index + batch_size]
                )
